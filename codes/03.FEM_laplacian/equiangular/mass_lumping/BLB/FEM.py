...
for bw in bws:
    ...
    B12_lumped_inverse = np.diag(np.sqrt(1./np.sum(B.array(), axis=1)))
    A_array = A.array()
    M = B12_lumped_inverse@A_array@B12_lumped_inverse
    M = scipy.sparse.csc_matrix(M)

    eig_values, eig_vectors = scipy.sparse.linalg.eigsh(M, N, which='SM', tol=1e-4)
