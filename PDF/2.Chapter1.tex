...
In figures \ref{fig:DeepSphere equivariance error} and \ref{fig:Optimal equivariance error} we plot the mean equivariance error 
...
\begin{figure}[h]
	...
	\caption{\label{fig:DeepSphere equivariance error}Mean equivariance error for $G$, $G'$ and the full HKGL, by spherical harmonic degree. Notice the difference in the scale of the y axis for DeepSphere, that reaches errors up to 30\%.}
\end{figure}

To conclude, we report in table \ref{tab:final results} as final metric for the evaluation of rotation equivariance of the two graphs $G$, $G'$ the mean equivariance error for a fixed $N_{side}=8$ that we computed by sampling random coefficients $\theta_\ell^m \in(0,1)$ of linear combinations of all the spherical harmonics up to degree $\ell=16$
$$f = \sum_{\ell\leq 15,\ |m|\leq\ell}\theta_\ell^m Y_\ell^m(x)
$$
and by averaging on random rotations $g\in SO(3)$, showing substantial improvements. 

\begin{table}
		\centering
\begin{tabular}{c|c}
	&Mean equivariance error $\overline{E}$\\\hline
	DeepSphere graph $G$ & 12.41\%\\
	Optimal graph $G'$ & 3.97\%
\end{tabular}
